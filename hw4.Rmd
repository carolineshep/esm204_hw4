...
# Establish 0.02 as the discount rate policy, while assuming society is risk neutral:
discount_rate_pol <- 0.02

# Policy A
# Select for the baseline of warming and damages and year, then use mutate to create new columns
warming_damages_a <- warming_damages %>%
  select(warming_baseline, damages_baseline, year) %>%
  mutate(damages_2 = damages_funct(1.5*warming_baseline)) %>% # 1.5X the baseline, per question instructions
  mutate(pv_baseline = damages_baseline*(1+discount_rate_pol)^(-(year-baseline_year))) %>%
  mutate(pv_2 = damages_2*(1+discount_rate_policy)^(-(year-baseline_year)))

# Defining and solving for policy values
policy_a_1 <- 0.5
policy_a_2 <- 0.5
expected_val_a_1 <- sum(warming_damages_policy_a$pv_baseline)
expected_val_a_2 <- sum(warming_damages_policy_a$pv_2)
expected_val_a <- policy_a_1*expected_val_a_1+policy_a_2*expected_val_a_2 
#This equation gives us the expected value from policy A 


#Policy B
warming_damages_b <- warming_damages %>%
  select(year, warming_baseline) 
for (i in 30:79){
  warming_damages_b$warming_baseline[i]=1.29
}

warming_damages_b <- warming_damages_policy_b %>%
  mutate(damages_b = damages_funct(warming_baseline)) %>%
  mutate(pv_b = damages_b*(1+discount_rate_policy)^(-(year-baseline_year)))
p_b <- 1.0
expected_val_b <- sum(warming_damages_policy_b$pv_b)*p_b #expected present value of damages up to 2100 policy B in dollars
...
**Figure 5:** The above plot shows the expected value of policies A and B as a function of the initial cost of implementing Policy B (Policy A has no initial costs).  <-- alter this figure caption
